...
Starting from the original question posed by the user, Palmisano looks for similar questions,
complete with their answers, in Q&A databases. Extracted questions need to be evaluated so
...
that remains vague and open to discussions or contradictions. \cite{Agirre:12}
recaps some of the issues encountered while annotating.


\subsection{Preprocessing steps}

Before feature extraction, sentence pairs are preprocessed with
the following rules:
\begin{itemize}
\item tokenization, Part-Of-Speech tagging and lemmatization. For the user
question, the necessary information is already provided by preceding stages of the Palmisano
pipeline. For questions extracted from databases the Stanford Core NLP pipeline
is invoked.
...
Except for the Stanford pipeline, no other external library is used during
this process.
...
types of coverages, which try to take into account other semantic properties
of the tokens in order to obtain a more realistic measure.


\subsubsection*{WordNet-augmented word overlap}

Here we try to take into account different words that have however
a quite close meaning. Such ``closeness'' is measured using the path
...
human annotations. In TakeLab, words and their contexts are expressed
by a term-document matrix. Word vectors are then summed together
...
This collects three features dealing entirely with numeric tokens,
...
First we build $N_{1}$ and $N_{2}$, two sets containing all the numerical tokens
respectively from the first and the second sentence. Then we compute the
following features:
...
\[
\frac{2|N_{1}\cap N_{2}|}{|N_{1}|+|N_{2}|}
\]


\[
N_{1}\subseteq N_{2}\vee\, N_{2}\subseteq N_{1}
\]

where the last is a boolean value. As we specified in the code, TakeLab requires that
numbers differing only for the number of decimal digits (e.g 6.2 and 6.23) be considered equal.
...
called STS.input.\textit{name}.txt, and another with the annotated similarity
scores, named STS.gs.\textit{name}.txt.
...
it is not really optimized for huge sequences of queries, so I applied
...
\item lack of details: whether punctuaction characters or numbers should be taken into account
for some features, how to manage different tagsets into the same application, etc... were deemed
too implementation-specific to be added to a mostly theorical paper. Even with some aid by the professors,
I cannot be completely sure I always made the best decisions.
\end{itemize}

\section{Applying this system in Palmisano}

From now on, I'm going to refer to the original TakeLab simple system.
It is my personal opinion that it will not be very suitable for being applied
...
the "W-words" of a question, for instance "where" versus "why", which means
asking for a location versus asking for a reason. The tags "types" and
"categories" added in the tweets to recognize different kinds of questions could
play a role too, although they are technically outside the sentence itself:
being able to differentiate immediately between a question asking for a fact
and one collecting personal opinions could be a good jump forward our goal.

Furthermore, when a sufficiently large number of tweets is available, both
the training and cross validation should be repeated on those. 
